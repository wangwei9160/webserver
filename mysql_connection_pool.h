...
public:
    connectionINIT(MYSQL **connect, mysql_connection_pool *connectpool);
    ~connectionINIT();

private:
    MYSQL *connectInit;
    mysql_connection_pool *poolINIT;
...
void mysql_connection_pool::init(string url,int port,string user,string password,string databasename,int Maxconect,int close_log){
    this->Url = url ;
    this->Port = port ;
    this->User = user ;
...
MYSQL *mysql_connection_pool::GetConnection(){
    MYSQL *conn = NULL;
    if(connectList.size() == 0) return NULL;
...
//创建连接。
connectionINIT::connectionINIT(MYSQL **connect, mysql_connection_pool *connectpool);{
    *SQL = connpool->GetConnection();
    connectInit = *SQL;
    poolINIT = connection_pool;
}
//释放连接
connectionINIT::~connectionINIT(){
    poolINIT->ReleaseConnection(connectionINIT);
}
